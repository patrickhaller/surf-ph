...
//static gboolean contextmenu(WebKitWebView *v, GtkMenu *m, gpointer user_data);
...
createwindow(WebKitWebView  *v, WebKitWebFrame *f, Client *c) {
	Client *n = newclient();
	XLowerWindow(dpy, GDK_WINDOW_XID(GTK_WIDGET(n->win)->window));
	return n->view;
}
...
destroyclient(Client *c) {
	...
	if(clients == NULL)
		...
		XRaiseWindow(dpy, GDK_WINDOW_XID(GTK_WIDGET(clients->win)->window));
}
...
keypress(GtkWidget* w, GdkEventKey *ev, Client *c) {
	...
	updatewinid(c);
	if (getenv("SURF_EMACS") == NULL) {
		if (ev->keyval == GDK_Escape) {
			is_input = FALSE;
			return TRUE;
		}
		if (ev->keyval == GDK_i && is_input == FALSE) {
			is_input = TRUE;
			return TRUE;
		}
		if (is_input == TRUE) {
		...
		for(i = 0; i < LENGTH(keys); i++) {
			if(gdk_keyval_to_lower(ev->keyval) == keys[i].keyval
					&& CLEANMASK(ev->state) == (keys[i].mod^MODKEY)
					&& keys[i].func) {
...
newclient(void) {
	...
	gtk_window_set_default_size(GTK_WINDOW(c->win), SURF_SIZE_X, SURF_SIZE_Y);
	...
	//g_signal_connect(G_OBJECT(c->view), "populate-popup", G_CALLBACK(contextmenu), c);
